...
export const createEclipseEditLendTransaction = async (
  createEditLendTransactionData: EditLendTransactionInterface,
) => {
  const { walletAddress, interest, offerId } = createEditLendTransactionData;
  ...
  if (!walletAddress || !interest) return undefined;
...
export const eclipseCancelLendTransaction = async (
  data: CancelLendTransactionInterface,
) => {
  const { walletAddress, offerId } = data;
